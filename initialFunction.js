...
function populateDB(tx) {	
    ...
    tx.executeSql('CREATE TABLE IF NOT EXISTS `cl_cours` ('+
        ...
        'PRIMARY KEY (`cours_id`))',[],null,exeError);
    ...
    tx.executeSql('CREATE TABLE IF NOT EXISTS `cl_en_course_description` ('+
    ...
    tx.executeSql('CREATE TABLE IF NOT EXISTS `cl_en_announcement` ('+
    ...
    tx.executeSql('CREATE TABLE IF NOT EXISTS `cl_en_calendar_event` ('+
    ...
    tx.executeSql('CREATE TABLE IF NOT EXISTS `cl_en_wrk_assignment` ('+
    ...
    tx.executeSql('CREATE TABLE IF NOT EXISTS `cl_en_wrk_submission` ('+
        ...
        '`score` smallint(3), PRIMARY KEY (`id`) )',[],null,exeError);              

    tx.executeSql('DROP TRIGGER IF EXISTS tr_i_wrk_submission',[],null,exeError);
    // tx.executeSql(
    //             'CREATE TRIGGER IF NOT EXISTS tr_i_wrk_submission AFTER INSERT ON cl_en_wrk_submission '+
    //             'FOR EACH ROW BEGIN INSERT INTO sync_change_logs VALUES '+
    //             ' (NULL,datetime(),\'cl_EN_wrk_submission\',\'id\',new.id,\'I\','+
    ...
    //             '    "vals": [\'||new.id||\', \'||new.assignment_id||\', 0, \'||new.user_id||\', '+
    //             '             0, "\'||new.title||\'","\'||new.visibility||\'", '+
    //             '            "\'||new.creation_date||\'", "\'||new.last_edit_date||\'", "\'||new.authors||\'", '+
    //             '             "\'||new.submitted_text||\'", "", "", "", 0]}\',\'-\',datetime(),1);'+
    //             'END;',[],null,exeError);
    ...
    //tx.executeSql('DROP TABLE IF EXISTS sync_change_logs',[],null,exeError);
    tx.executeSql('CREATE TABLE IF NOT EXISTS sync_change_logs ('+
        'id INTEGER PRIMARY KEY AUTOINCREMENT, session_id char(45) NOT NULL,'+
        ...
        'valid tinyint(1) NOT NULL )',[],null,exeError);
    ...
    tx.executeSql('CREATE TABLE IF NOT EXISTS sync_maps ('+
        ...
        'luid varchar(30) NOT NULL, guid int(30) NOT NULL, PRIMARY KEY (`id`) )',[],null,exeError);
    
    tx.executeSql('CREATE TABLE IF NOT EXISTS sync_sessions ('+
        'id integer NOT NULL PRIMARY KEY,'+
        'sessionid integer NOT NULL,'+
        'messageid integer NOT NULL,'+
        'anchorid integer NOT NULL)',[],null,exeError);
}
...
//        " NULL, 3, 'enable', 0);");
    ...
    //     " VALUES (5, 'Fahri', 'Firdausillah', 'admin', '123',"+
    //     " '', 'claroline', 'fahri@qudsiyyah.com', '', '', '', NULL, 2, 0, 0);", [], dbSuccess, exeError);
